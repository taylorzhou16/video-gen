...
| Kling | shot-level（首帧图） | 逐 shot 调用 | N 个视频片段 |
| Veo3 | shot-level | 逐 shot 调用 | N 个视频片段 |

**Kling-Omni 必须按 shot-level 执行**：
1. 为每个 shot 生成分镜图：`generated/frames/{shot_id}_frame.png`
...
| `image_urls` | 数组 | 最多 12 张参考图 |
| `video_urls` | 数组 | 最多 1 个视频参考（omni_reference） |
| `audio_urls` | 数组 | mp3/wav，≤15s（omni_reference） |
