...
当 Kling API 遇到 429（并发限制）、402（余额不足）、或其他不可恢复错误时，需要降级到 Path B。

### 降级前提条件

**必须满足以下条件才能降级**：
1. 用户明确同意降级
2. 降级后仍有可用后端（如 Kling 官方 API 可用但 Omni 不可用）
3. 降级后的模式能满足基本需求（场景可控）
...
| `omni-video` (kling-omni) | `text2video` (kling) | Omni → Kling | 失去角色一致性，纯文字生成 |
| `img2video` (kling) | `text2video` (kling) | 无变化 | 失去首帧控制 |

**禁止的降级**：
- ❌ `omni-video` → Vidu text2video（Vidu 不支持 image_list）
- ❌ `img2video` → Vidu text2video（Vidu 的 img2video 是单图参考，非首帧控制）

### Path A → Path B 降级流程

当 omni-video 无法执行时，降级到 Path B（分镜图 + Kling img2video）：
...
可选方案：
A. 等待并重试（可能需要较长时间）
B. 降级到 Path B：先生成分镜图，再用 Kling img2video（角色一致性会降低）
C. 取消本次生成
