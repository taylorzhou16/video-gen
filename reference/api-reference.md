...
# Seedance 2 自动组装模式（推荐：从 storyboard 自动计算时间分段、拼装 prompt、排列 image_urls）
...
# Seedance 2 手动模式（兜底）
python ~/.claude/skills/video-gen/video_gen_tools.py video --backend seedance --prompt "时间分段 prompt..." --image-list frame.png ref.jpg --duration 8 --output out.mp4

# Seedance 2 首尾帧控制模式
python ~/.claude/skills/video-gen/video_gen_tools.py video --backend seedance --mode first_last_frames --image-list <首帧图> <尾帧图> --prompt "动作描述" --duration 5 --output out.mp4
...
| YUNWU_API_KEY | Gemini 图片生成（yunwu 代理） | 图片生成时（最低优先级备用） |
